/* Pracownia Tortów — pure calculation engine, all money in PLN. */
...
export function validateSale(s){
  const text=(key,max,required=false)=>{if(typeof s[key]!=='string'||s[key].length>max||(required&&!s[key].trim()))throw Error('Zamówienie: nieprawidłowe pole '+key+'.');};
  if(!s||typeof s!=='object')throw Error('Nieprawidłowe zamówienie.');
  ...
  for(const key of ['createdAt','updatedAt'])if(typeof s[key]!=='string'||!Number.isFinite(Date.parse(s[key])))throw Error('Nieprawidłowa data zapisu.');
  for(const key of ['price','deposit','paid']){finite(s[key],key);if(Math.abs(s[key]*100-Math.round(s[key]*100))>1e-5)throw Error('Kwoty podaj z dokładnością do groszy.');}
...
export function validateRecipeSteps(recipe){
  if(recipe.steps===undefined)return;
  if(!Array.isArray(recipe.steps)||recipe.steps.length>100)throw Error('Receptura może zawierać do 100 kroków.');
  for(const step of recipe.steps){
    if(!step||typeof step.title!=='string'||!step.title.trim()||step.title.length>200||typeof step.text!=='string'||!step.text.trim()||step.text.length>12000)throw Error('Każdy krok musi mieć tytuł i instrukcję.');
    if(!Array.isArray(step.groups)||step.groups.some(g=>typeof g!=='string'||!recipe.rows.some(r=>r.group===g)))throw Error('Krok odwołuje się do nieistniejącej sekcji składników. Wybierz sekcję ponownie.');
    if(step.minutes!==null&&(typeof step.minutes!=='number'||!Number.isFinite(step.minutes)||step.minutes<=0||step.minutes>10080))throw Error('Czas kroku musi być dodatnią liczbą minut (do 10080) lub pozostać pusty.');
    if(step.temperature!==null&&(typeof step.temperature!=='number'||!Number.isFinite(step.temperature)||step.temperature< -30||step.temperature>300))throw Error('Temperatura musi być liczbą od −30 do 300°C lub pozostać pusta.');
  }
}
