...
export function cookingPlan(snapshot){
  ...
  snapshot.order.tiers.forEach((tier,index)=>{
    ...
    steps.push({title:'Odmierz składniki',text:'Przygotuj osobne naczynia dla poszczególnych sekcji. Ilości poniżej są już przeliczone dla tego piętra i całej zamówionej liczby tortów dokładnie według współczynnika z torty.html. Zapas zakupowy nie jest do nich doliczany.',minutes:null,temperature:null,rows,tier:index+1,recipe:recipe.name,kind:'preparation'});
...
export function createCookingUI(api){
  ...
  const esc=api.esc;
  function ingredients(rows){return rows.length?`<div class="cook-ingredients">${rows.map(r=>`<div><span>${esc(r.name)}<small>${esc(r.group)}</small></span><strong>${api.fmt(r.net,2)} ${r.unit}</strong></div>`).join('')}</div>`:'<p class="help">Ten krok nie wymaga dodania kolejnych składników.</p>';}
